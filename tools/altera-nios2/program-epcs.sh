...
do
    case "$1" in
        ...
        --help)
            ...
            echo " --offset [FLASH_OFFS] .. Specify offset within flash where the image"
                 "                          shall be programmed to."
...
# Relocate image to other offset if necessary
if [ -n "${OFFSET}" ];
then
    echo
    echo "INFO: The flash image will be generated for flash offset ${OFFSET}."
    ...
    # Convert binary back to srec, but shift it to the given offset
    echo
    echo "INFO: Convert binary back to flash file, but shift it to offset ${OFFSET}..."
    CMD="bin2flash --input=${IMG_FILE}.bin --output=${IMG_FILE}.flash --location=${OFFSET}"
    ${CMD} || {
        echo
        echo "ERROR: Shifting image to the given offset ${OFFSET} failed!"
        exit 1
    }
fi
